<script lang="ts">
  ...
  export let item: ICardData;

  $: bookedText = item.isBooked ? "Забронировано" : "Доступно";
  $: buttonText = item.isBooked ? "Убрать бронь" : "Забронировать";
  $: bookedClass = item.isBooked ? "text-red-600" : "text-green-600";
  $: actionClass = item.isBooked ? "opacity-50 hover:opacity 100" : "";
  $: actionHandler = item.isBooked ? handleUnbooking : handleBooking;
